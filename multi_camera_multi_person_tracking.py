...
class DetThreadBody:
    def __init__(self, det, frames, max_queue_length=2):
        self.process = True
    ...
    def __call__(self):
        while self.process:
            if self.detection_queue.qsize() > self.max_queue_length:
                time.sleep(0.1)
            try:
                frames = self.frames.get_nowait()
            except queue.Empty:
                frames = 'Empty'
            if frames == 'Empty':
                continue
            elif frames is None and self.detection_queue.empty():
                self.process = False
                break
            det = self.detector.run_async(frames)
            self.detection_queue.put([frames, det])
...
def run(params, config, capture, detector, reid):
    ...
    det_body = DetThreadBody(
        detector, thread_body.frames_queue, max_queue_length=len(capture.captures) * 2)
    frames_thread = Thread(target=thread_body)
    det_thread = Thread(target=det_body)
    frames_thread.start()
    det_thread.start()
    ...
    if len(params.output_video):
        ...
        output_video = cv.VideoWriter(
            params.output_video, fourcc, min(fps), video_output_size)
    else:
        output_video = None
    ...
    while thread_body.process:
        ...
        start = time.time()
        try:
            [prev_frames, all_detections] = det_body.detection_queue.get_nowait()
        except queue.Empty:
            all_detections = None
        ...
        vis = visualize_multicam_detections(
            prev_frames, tracked_objects, fps, **config['visualization_config'])
        if not params.no_show:
...
def main():
    ...
    if args.detections:
        person_detector = DetectionsFromFileReader(
